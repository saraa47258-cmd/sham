 ...
 * محسّن مع Virtual Scrolling و Lazy Loading
 */
...
const VirtualScroller = {
    itemHeight: 120, // ارتفاع كل عنصر بالبكسل
    ...
    init(containerId, items, renderFn) {
        ...
        // إذا كان عدد العناصر قليل، لا حاجة للـ Virtual Scrolling
        if (items.length < 20) {
            this.container.innerHTML = items.map((item, i) => renderFn(item, i)).join('');
            return;
        }
    ...
    setupVirtualScroll() {
        ...
        // إنشاء حاوية داخلية بالارتفاع الكامل
        this.container.innerHTML = `<div class="virtual-content" style="height: ${totalHeight}px; position: relative;"></div>`;
    ...
    onScroll() {
        ...
        this.content.innerHTML = visibleItems.map((item, i) => {
            ...
            return `<div style="position: absolute; top: ${top}px; left: 0; right: 0; height: ${this.itemHeight}px;">
                ${this.renderFn(item, actualIndex)}
            </div>`;
        }).join('');
    }
};
